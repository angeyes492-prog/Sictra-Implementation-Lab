...
def receipt(value=None, **changes):
    ...
    base = ExternalObservationReceipt(
        "RECEIPT-001", "0.1.0", value.fixture_id, fixture_fingerprint(value), value.fixture_author_id,
...
class E01ExternalObservationTests(unittest.TestCase):
    ...
    def test_self_review_exposure_and_confounder_invalidate(self):
        ...
        result = assess_external_observation(value, assess_fixture(value), receipt(
            value, reviewer_id=value.fixture_author_id, thesis_exposed_before_observation=True,
    ...
    def test_receipt_cannot_substitute_fixture_author_to_mask_self_review(self):
        value = fixture()
        result = assess_external_observation(value, assess_fixture(value), receipt(
            value, fixture_author_id="SUBSTITUTED-AUTHOR", reviewer_id=value.fixture_author_id,
        ))
        self.assertEqual("INVALID_TRIAL", result.disposition)
        self.assertEqual(("FIXTURE_AUTHOR_MISMATCH", "REVIEWER_IS_FIXTURE_AUTHOR"), result.reasons)
