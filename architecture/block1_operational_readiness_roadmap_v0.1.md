...
| 8. Operating plane | Make the system safe for multiple people and sustained use. | Local launcher initializes a signed-reader key/store layout under `%LOCALAPPDATA%`, outside the OneDrive repository, and validates it on reopen; backup/restore and ACL proof remain absent. | Named operator, documented local startup, backup/restore and access boundary. | SSO/RBAC, tenancy, secret manager, deployment controls, observability, backups, security review and disaster recovery. |
