...
| 5. Intelligence runtime | Form research questions, detect changes, contradictions and bounded insights. | A durable dossier store now converts a synthetic attested-input delta into separately typed facts, empty interpretations/hypotheses, uncertainties and questions; the real workbook has only one release and no real delta dossier. | A durably bound real-source run produces facts, interpretations and uncertainties separately. | Load/performance SLOs and continuous regression with real anonymised cases. |
...
| 7. Watchlists and cadence | Recheck approved sources and surface meaningful change without noise. | Atomic manual cycle now has a bridge that admits exactly one current attested source record; no scheduler, version-selection policy or retained operator cycle. | Defined cadence and a manually triggered, attested delta report for one source. | Budgeted workers, rate limits, change detection, alerts, kill switch and incident runbook. |
| 8. Operating plane | Make the system safe for multiple people and sustained use. | Local single-user service; signed reader requires programmatic injection and has no operator key configuration yet. | Named operator, documented local startup, backup/restore and access boundary. | SSO/RBAC, tenancy, secret manager, deployment controls, observability, backups, security review and disaster recovery. |
